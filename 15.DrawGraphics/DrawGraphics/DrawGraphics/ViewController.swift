...
class ViewController: UIViewController {
    ...
    @IBAction func btnDrawLine(_ sender: UIButton) {
        ...
        context.setStrokeColor(UIColor.blue.cgColor)  // 선 색상을 파린색으로 설정
        
        context.move(to: CGPoint(x: 170, y: 200))  // 커서의 위치를 (170, 200)으로 이동
        context.addLine(to: CGPoint(x: 270, y: 350))  // 시작 위치에서 (270, 350)까지 선 추기
        context.addLine(to: CGPoint(x: 70, y: 350))  // 이전 위치에서 (70, 350)까지 선 추가
        context.addLine(to: CGPoint(x: 170, y: 200))  // 이전 위치에서 (170, 200)까지 선 추가
        context.strokePath()  // 추가한 선을 콘텍스트에 그림
